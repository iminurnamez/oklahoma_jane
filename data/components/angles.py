...
def get_angle(origin, destination):
    """Returns angle in radians from origin to destination.
        ...
        return pi/4 (45 deg) rather than 
        """
...
def get_collision_sides(rect, other_rect):
    """Finds whether collision is left/right, top/bottom or corner."""
    thickness = 1
    width, height = rect.size
    left_bumper = pg.Rect(rect.left - thickness, rect.top, thickness, height) 
    right_bumper = pg.Rect(rect.right, rect.top, thickness, height)
    top_bumper = pg.Rect(rect.left, rect.top - thickness, width, thickness)
    bottom_bumper = pg.Rect(rect.left, rect.bottom, width, thickness)
